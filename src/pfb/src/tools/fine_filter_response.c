...
int main(int argc, char *argv[]) {
	...
	const double freqStart = 0;
	double freqSamples = genFreq/freqStep;
	fprintf(stdout, "freq samples: %f\n", freqSamples);
    //Generate freq array.
    double* freq = NULL;
    freq = (double*) malloc(freqSamples*sizeof(double));
    freq[0] = freqStart;
    for(i = 1; i <= freqSamples; i++) {
        freq[i] = freq[i-1] + freqStep;
	...
	fprintf(stdout,
			...
			"\tChannels:\t %d\n"
			"\tElements:\t %d\n",
			samples, fs, genFreq, freqStep, coarseChannels, elements);

	for(f = 0; f < freqSamples; f++) {
		for(n = 0; n < samples; n++) {
